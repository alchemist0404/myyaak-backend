        ...
        if (!onError) {
            onError = function (err) {
                ...
                if (err.code === 1) {
                    ...
                    console.log('Please activate Geolocation and refresh the page. If it is already active, please check permissions for this website.');
                    return;
                }

                if (err.code === 3) {
                    console.log('Cannot retrieve GPS position. Signal is absent.');
                    return;
                }
            };
        }
...
var oldVal = this._computeCompassHeading(event.alpha, event.beta, event.gamma)  
var degree = this.heading  
var smoothing = 0.9
    if(oldVal - degree < -180){
        // Invert the logic
      this.heading = oldVal - smoothing * (oldVal + 360 - degree);
      if (smoothedValue < 0){
        this.heading = this.heading + 360;
      }
    } else if (oldVal - degree > 180){
        this.heading = oldVal + (360 + degree - oldVal) * smoothing;
      if (this.heading > 360){
        this.heading = this.heading - 360;
      }
    }
    else {
      this.heading = oldVal + smoothing * (degree - oldVal);
    }   
    return this.heading ;
			}else{
